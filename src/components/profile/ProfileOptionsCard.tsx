import { Text, View } from 'react-native';
import * as WebBrowser from 'expo-web-browser';
import { useTranslation } from 'react-i18next';
import { SymbolView } from '../shared/SymbolView';
import { GlassicView } from '../shared/GlassicView';
import { BouncyPressable } from '../shared/BouncyPressable';
import { useProfileStyles } from '../../styles/ProfileStyles';
import { useTheme } from '../../hooks/useTheme';

interface ProfileOptionsCardProps {
  onSubScreenNavigate: (screen: 'change-email' | 'blocked-users' | 'support' | 'privacy' | 'about' | 'reports' | 'theme') => void;
  onLogout: () => void;
  onDeleteAccount: () => void;
}

// بطاقة الخيارات - تغيير البريد والدعم والخصوصية والمظهر والخروج
export const ProfileOptionsCard: React.FC<ProfileOptionsCardProps> = ({
  onSubScreenNavigate,
  onLogout,
  onDeleteAccount,
}) => {
  const { t } = useTranslation();
  const styles = useProfileStyles();
  const { colors } = useTheme();

  return (
    <GlassicView cornerRadius={24} style={styles.optionsCard}>
      <Text style={styles.optionsTitle}>{t('profile.settings_card_title')}</Text>
      
      <BouncyPressable style={styles.optionRow} onPress={() => onSubScreenNavigate('change-email')}>
        <Text style={styles.chevron}>‹</Text>
        <View style={styles.optionRightContainer}>
          <Text style={styles.optionLabel}>{t('profile.change_email')}</Text>
          <View style={styles.optionIconWrapper}>
            <SymbolView
              name={{ ios: 'envelope.fill', android: 'mail' }}
              size={20}
              tintColor={colors.brand.gold}
            />
          </View>
        </View>
      </BouncyPressable>

      <BouncyPressable style={styles.optionRow} onPress={() => onSubScreenNavigate('blocked-users')}>
        <Text style={styles.chevron}>‹</Text>
        <View style={styles.optionRightContainer}>
          <Text style={styles.optionLabel}>{t('profile.blocked_users')}</Text>
          <View style={styles.optionIconWrapper}>
            <SymbolView
              name={{ ios: 'nosign', android: 'block' }}
              size={20}
              tintColor={colors.brand.gold}
            />
          </View>
        </View>
      </BouncyPressable>

      <BouncyPressable style={styles.optionRow} onPress={() => onSubScreenNavigate('reports')}>
        <Text style={styles.chevron}>‹</Text>
        <View style={styles.optionRightContainer}>
          <Text style={styles.optionLabel}>{t('profile.reports')}</Text>
          <View style={styles.optionIconWrapper}>
            <SymbolView
              name={{ ios: 'exclamationmark.shield.fill', android: 'security' }}
              size={20}
              tintColor={colors.brand.gold}
            />
          </View>
        </View>
      </BouncyPressable>

      <BouncyPressable style={styles.optionRow} onPress={() => onSubScreenNavigate('support')}>
        <Text style={styles.chevron}>‹</Text>
        <View style={styles.optionRightContainer}>
          <Text style={styles.optionLabel}>{t('profile.support')}</Text>
          <View style={styles.optionIconWrapper}>
            <SymbolView
              name={{ ios: 'bubble.left.and.bubble.right.fill', android: 'forum' }}
              size={20}
              tintColor={colors.brand.gold}
            />
          </View>
        </View>
      </BouncyPressable>

      <BouncyPressable style={styles.optionRow} onPress={() => onSubScreenNavigate('privacy')}>
        <Text style={styles.chevron}>‹</Text>
        <View style={styles.optionRightContainer}>
          <Text style={styles.optionLabel}>{t('profile.privacy')}</Text>
          <View style={styles.optionIconWrapper}>
            <SymbolView
              name={{ ios: 'shield.fill', android: 'shield' }}
              size={20}
              tintColor={colors.brand.gold}
            />
          </View>
        </View>
      </BouncyPressable>

      <BouncyPressable
        style={styles.optionRow}
        onPress={() => WebBrowser.openBrowserAsync('https://atharweb.elcomlab.site')}
      >
        <Text style={styles.chevron}>‹</Text>
        <View style={styles.optionRightContainer}>
          <Text style={styles.optionLabel}>{t('profile.website')}</Text>
          <View style={styles.optionIconWrapper}>
            <SymbolView
              name={{ ios: 'globe', android: 'language' }}
              size={20}
              tintColor={colors.brand.gold}
            />
          </View>
        </View>
      </BouncyPressable>

      <BouncyPressable style={styles.optionRow} onPress={() => onSubScreenNavigate('about')}>
        <Text style={styles.chevron}>‹</Text>
        <View style={styles.optionRightContainer}>
          <Text style={styles.optionLabel}>{t('profile.about')}</Text>
          <View style={styles.optionIconWrapper}>
            <SymbolView
              name={{ ios: 'info.circle.fill', android: 'info' }}
              size={20}
              tintColor={colors.brand.gold}
            />
          </View>
        </View>
      </BouncyPressable>

      <BouncyPressable style={styles.optionRow} onPress={() => onSubScreenNavigate('theme')}>
        <Text style={styles.chevron}>‹</Text>
        <View style={styles.optionRightContainer}>
          <Text style={styles.optionLabel}>{t('profile.theme')}</Text>
          <View style={styles.optionIconWrapper}>
            <SymbolView
              name={{ ios: 'paintbrush.fill', android: 'palette' }}
              size={20}
              tintColor={colors.brand.gold}
            />
          </View>
        </View>
      </BouncyPressable>

      <BouncyPressable style={styles.optionRow} onPress={onLogout}>
        <Text style={styles.chevronRed}>‹</Text>
        <View style={styles.optionRightContainer}>
          <Text style={styles.optionLabelRed}>{t('profile.logout')}</Text>
          <View style={styles.optionIconWrapper}>
            <SymbolView
              name={{ ios: 'arrow.right.square.fill', android: 'logout' }}
              size={20}
              tintColor="#DC2626"
            />
          </View>
        </View>
      </BouncyPressable>

      <BouncyPressable style={[styles.optionRow, styles.lastOptionRow]} onPress={onDeleteAccount}>
        <Text style={styles.chevronRed}>‹</Text>
        <View style={styles.optionRightContainer}>
          <Text style={styles.optionLabelRed}>{t('profile.delete_account')}</Text>
          <View style={styles.optionIconWrapper}>
            <SymbolView
              name={{ ios: 'trash.fill', android: 'delete' }}
              size={20}
              tintColor="#DC2626"
            />
          </View>
        </View>
      </BouncyPressable>
    </GlassicView>
  );
};
